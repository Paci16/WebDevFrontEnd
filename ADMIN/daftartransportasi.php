...
<form method="POST">
    <div class="form-group row mb-2">
        <label for="search"class="col-sm-2">Transportasi</label>
        
        <!--untuk panjang box isi nya-->
        <div class="col-sm-6">
            <input type="text" name="search" class="form-control" id="search" 
            value="<?php if(isset($_POST["search"]))
            {echo $_POST["search"];}?>"placeholder="Cari Kode Transportasi">
        </div>
                ...
                <tbody>
                    ...
                    if(isset($_POST["kirim"]))
                    {
                        ...
                        $query = mysqli_query($connection, "select * 
                            from transportasi, travel
                            where KODEtransport like '%".$search."%' AND travel.KODEtravel = transportasi.KODEtravel"); //ini diambil dari dua tabel
                    }else

                    {
                        //akan membuat dia tidak redudansi
                        $query = mysqli_query($connection, "select * FROM transportasi, travel
                        WHERE travel.KODEtravel = transportasi.KODEtravel");
                    }
