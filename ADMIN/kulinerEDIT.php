...
<?php 
	...
	if(isset($_POST['edit']))
	{
            ...
            if(($ekstensifile == "jpg") or ($ekstensifile == "JPG") or ($ekstensifile == "png") or ($ekstensifile == "PNG"))
		{
			...
			mysqli_query($connection, "UPDATE kuliner SET 
			lokasiKULINER = '$lokasikuliner', fotoKULINER = '$namafile' 
			WHERE namaKULINER = '$namakuliner'");
		}
	} else {
		mysqli_query($connection, "UPDATE kuliner SET 
		lokasiKULINER = '$lokasikuliner' WHERE namaKULINER = '$namakuliner'");
	}

	}

$kodefoto = $_GET["ubah"];
$editfoto = mysqli_query($connection, "SELECT * FROM kuliner 
WHERE namaKULINER = '$kodefoto'");
...
<div style="padding-left: 0; margin-bottom: 15px;" class="container">
			<h4 class="display-8">Input data kuliner</h4>
		</div>
	</div>
	...
	<form method="POST" enctype="multipart/form-data">
		<div class="form-group row">
			...
			<div class="col-sm-10">
				...
				value="<?php echo $roweditfoto['namaKULINER']; ?>" readonly>
			</div>
		</div>
		...
		<div class="form-group row">
			...
			<div class="col-sm-10">
				...
				value="<?php echo $roweditfoto['lokasiKULINER']; ?>">
			</div>
		</div>
		...
		<div style="margin-top: 10px;" class="form-group row">
			...
			<div class="col-sm-10">
				<td>
    	<?php if (!empty($row['fotoKULINER']) && is_file("images/".$row['fotoKULINER'])): ?>
        	<img src="images/<?php echo $row['fotoKULINER']?>" width="80" alt="Current Image">
    	<?php endif; ?>
	</td>
